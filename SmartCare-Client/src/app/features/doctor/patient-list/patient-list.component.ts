...
export class PatientListComponent {
  ...
  private search = toSignal(this.searchControl.valueChanges.pipe(startWith('')), {
    initialValue: '',
  });
  ...
  protected patients = computed<PatientSummary[]>(() => {
    ...
    return [...map.values()].sort((a, b) => b.lastVisitDate.localeCompare(a.lastVisitDate));
  });

  protected filtered = computed<PatientSummary[]>(() => {
    const term = this.search().trim().toLowerCase();
    if (!term) return this.patients();
    return this.patients().filter((p) => p.patientName.toLowerCase().includes(term));
  });
